# PPV-Pareto v9 — End-to-end forecast pipeline with harmonization, row-wise fallback, calibration, EC variants, and grouped CV/cross-fitted calibration.
# Additions in this version:
# - GroupKFold CV by year (no leakage)
# - Cross-fitted isotonic calibration (OOF-based), then refit final models
# - CV metric panel (AUC + Brier)
# - Directional sanity check for key variable(s)
# - Repro artifacts: knee_selection.json, 2024_variable_sources.json, RUN_METADATA.json
...
STATE_MIN_WEIGHT = 0.0  # set >0 to collapse rare states (by total weight) to "_OTHER_"
SAVE_CALIBRATION_PLOTS = True
SAVE_OOF_PANELS = True
...
# ADD near other helpers (e.g., after cv_metric_panel or nearby)
...
def main():
    ...
    # Ensure files exist
    for pth in [DATA_2012, DATA_2016, DATA_2020, DATA_2024]:
        if not _exists(pth):
    ...
    # sort by auc desc, keep unique by tuple(features)
    ...
    for m in models:
        ...
        try:
            pipe, calibrator, n_oof, data_used = cross_fit_isotonic(df_all, feats, C, n_splits=N_SPLITS) if CROSS_FIT_ISOTONIC else (build_pipe(feats, C), None, 0, None)
            if not CROSS_FIT_ISOTONIC:
            ...
            print(f"Trained model [{name}] with cross-fitted isotonic | OOF n={n_oof} | feats={feats} | C={C:.4g} | AUC≈{m['auc']:.3f}")
    ...
    # Shrunk state-refit EC
    if ec_refit is not None:
        feats_ec = ec_refit["features"]
        mask_rows_ec = X24[feats_ec].notna().all(axis=1) & w24.notna()
        p_ec = np.full(len(X24), np.nan, dtype=float)
        if mask_rows_ec.any():
            raw = ec_refit["pipe"].predict_proba(X24.loc[mask_rows_ec, feats_ec])[:,1]
            p_ec[mask_rows_ec.values] = np.clip(ec_refit["cal"].transform(raw), 0.0, 1.0)
        sdf_refit_shr, ev_refit_shr, ev_refit_thr05, total_ev_refit = state_level_ec_from_preds_shrunk(
            p_ec, w_vec, X24["state"].values if "state" in X24.columns else np.full(len(X24), np.nan),
            min_neff=EC_MIN_NEFF, k_prior=EC_SHRINKAGE_K_PRIOR, reliability_threshold=EC_MIN_NEFF,
            ci_reps=BOOTSTRAP_REPS, ci_seed=BOOTSTRAP_SEED
        )
        ec_outputs.append((f"EC (state-refit + shrink; k_prior={EC_SHRINKAGE_K_PRIOR:g})", sdf_refit_shr, ev_refit_shr, ev_refit_thr05, total_ev_refit))
    ...
    with open(os.path.join(OUT_DIR,"RUN_METADATA.json"),"w") as f: json.dump(run_meta, f, indent=2)
    ...
    for label, sdf, ev_mean, ev_thr05, total_ev_present in ec_outputs:
        ...
        excluded = sdf.loc[~sdf["eligible"], "state"].tolist()
        if low_reli:
            lines.append(f"  • Low reliability (LOW_Neff): {', '.join(low_reli)}")
        if excluded:
            lines.append(f"  • Excluded from EV tally (neff < {EC_MIN_NEFF:g}): {', '.join(excluded)}")
    lines.append("")
    lines.append("Notes:")
    lines.append("- ANES is a national survey; state estimates are illustrative without small-area modeling (e.g., MRP).")
    lines.append("- We apply empirical-Bayes shrinkage of state means toward the national mean to stabilize thin samples.")
    ...
    lines.append(f"- 2024 white_nonhispanic derived from {VAR_2024_PREFS['race_eth_summary'][0]} if present; else from race+Hispanic.")
    lines.append(f"- State variable chosen by coverage+cardinality from candidates {VAR_2024_PREFS['state_candidates']}.")
    if SAVE_CALIBRATION_PLOTS:
        lines.append("- Calibration plots saved: calibration_knee.png, calibration_ec.png")
